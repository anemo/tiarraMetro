...
class Cookie
{
  ...
  //Cookieをセットするメソッド
  static public function set($name,$value,$pass,$time = 0, $path = '/', $server)
  {
    if (is_null($server))
    ...
    setcookie($name,base64_encode($encryptValue),$time , $path, $server);
     
    //暗号化された値を解読するためのキーをCookieに書き込みます。
    setcookie("iv_" . $name ,base64_encode($iv),$time , $path, $server);
  }
  ...
  //Cookieを削除する為のメソッド
  static public function delete($name,$path,$server){
    if (is_null($server))
      $server = $_SERVER['SERVER_NAME'];

    if (isset($_COOKIE['iv_' . $name]) && isset($_COOKIE[$name])) {
      setcookie( $name, null, Cookie::$endExpire, $path, $server );
      setcookie( 'iv_'.$name, null, Cookie::$endExpire, $path, $server );
    }
  }
